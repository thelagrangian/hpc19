...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  /* 
 * Points regarding threads accomplishing multiple tasks with multiple mutex locks:
 * 1. Within an individual thread/activation function, the order of unlockings should be exactly the opposite to
 * the order of lockings.
 * 2. Among threads/activation functions, the order of lockings should be the same to avoid deadlock.
 */
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      for (i=0; i<N; i++)
        a[i] += b[i];
      //omp_unset_lock(&locka);  //commented out
      omp_unset_lock(&lockb);
      omp_unset_lock(&locka);    //added
      }
    }  /* end of sections */
